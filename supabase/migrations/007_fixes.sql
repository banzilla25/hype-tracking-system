-- ============================================================
-- HYPE Tracking System — Migration 007
-- Fixes (semua idempotent — aman dijalankan ulang):
-- 1. Fix pois_insert RLS (freelancer sekarang bisa insert POI baru)
-- 2. Re-apply transition_claim_status lengkap (11 param, Fase 1 & 2)
-- 3. Re-apply source_poi_internal
--
-- Run: Supabase Dashboard → SQL Editor → paste & Run
-- ============================================================


-- ── 1. Fix pois_insert policy ─────────────────────────────────────────────
-- Policy lama hanya mengizinkan 'internal'. Freelancer perlu insert POI
-- baru (dari halaman "Sudah Approach?" & addPoiToPool). Validasi state
-- machine dilakukan di server action.
...
-- Grant eksekusi ke user yang terautentikasi
...
GRANT EXECUTE ON FUNCTION public.source_poi_internal TO authenticated;
